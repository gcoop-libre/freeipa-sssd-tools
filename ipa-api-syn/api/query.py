    ...
    def get(self, userid, recordnum=1):
        ...
        with sqlite3.connect(current_app.config["DBPROCS"]) as con:
                ...
                return_list.append(
                    {
                        "item": str(count),
                        "hash": row[0],
                        "timestamp": datetime.datetime.fromtimestamp(row[1]).strftime(
                            "%Y-%m-%d %H:%M:%S"
                        ),
                        "account": row[2],
                        "action": row[3],
            ...
            if len(return_list) > 0:
                ...
                return jsonify(
                    {"retval": "OK", "items": str(count), "result": return_list}
                )

            else:
                logging.warning("op=qry acct=" + userid + " status=account not found")
                return jsonify({"retval": "Error", "result": "Account not found"})
